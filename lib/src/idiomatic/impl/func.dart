...
SqlType aliasToSqlType(String alias) {
  ...
  switch (aliasUpper) {
    ...
    default:
      throw Exception("unsupported type $alias");
  }
}
...
ConverterSql getFromSqlConverter(SqlType sqlType, Type reflectedType) {
  switch (reflectedType) {
    case String:
      return _stringSqlConverter;

    case int:
      return _getIntConverter(sqlType);

    case double:
    case num:
      return _getDoubleConverter(sqlType);

    case DateTime:
      return _getDateTimeConverter(sqlType);

    default:
      return _getEntityConvertor(sqlType,
          reflectedType); // throw UnsupportedError("do not convert from sql for type: $reflectedType");
  }
}
...
ConverterSql getToSqlConverter(SqlType sqlType, Type reflectedType) {
  ...
  switch (sqlType) {
    case SqlType.INTEGER:
      return _getConverterToSqlInt(reflectedType);

    case SqlType.TEXT:
      return _stringSqlConverter;

    case SqlType.REAL:
      return _getConverterToSqlReal(reflectedType);

    default:
      throw UnsupportedError("do not convert to sql for sqltype: $sqlType");
  }
}
...
ConverterSql _getConverterToSqlInt(Type reflectedType) {
  switch (reflectedType) {
    ...
    case double:
    case num:
      return _intFromNumConverter;

    case String:
      return _intFromStringConverter;

    case DateTime:
      return _intFromDateTimeConverter;

    default:
      throw UnsupportedError("do not convert to sql INTEGER for type: $reflectedType");
  }
}
...
ConverterSql _getConverterToSqlReal(Type reflectedType) {
  switch (reflectedType) {
    ...
    case double:
    case num:
      return _doubleFromNumConverter;

    case String:
      return _doubleFromStringConverter;

    case DateTime:
      return _doubleFromDateTimeConverter;

    default:
      throw UnsupportedError("do not convert to sql REAL for type: $reflectedType");
  }
}
...
Object _dateTimeFromSqlConverter(Object sqlValue) {
  ...
  throw UnsupportedError(
      "do not convert value: $sqlValue with value type ${sqlValue.runtimeType} to DateTime");
}
